...
function AutoChatLog_UpdateLogs()
    ...
    if _G.AutoChatLogData == nil or #_G.AutoChatLogData == 0 then
        -- print("Keine Logs verfügbar.") -- Debug-Ausgabe
        logBox:SetText("Keine Logs verfügbar.")
    else
        local logText = table.concat(_G.AutoChatLogData, "\n")
...
function AutoChatLog_UpdateLogs()
    ...
    if not _G.AutoChatLogData or #_G.AutoChatLogData == 0 then
        logBox:SetText("Keine Logs verfügbar.")
    else
        local logText = table.concat(_G.AutoChatLogData, "\n")
        logBox:SetText(logText)
